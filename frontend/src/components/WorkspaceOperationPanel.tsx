...
export default function WorkspaceOperationPanel({
    ...
    const load = async () => {
        ...
        const activeExt = extOf(activePath);
        // JIT SDK formats: skip backend preview, render via frontend DocumentViewer
        if (JIT_DOC_EXTS.has(activeExt)) {
            setPreview({ type: activeExt.slice(1), kind: activeExt.slice(1) });
        ...
        try {
            const [data, revs] = await Promise.all([
                fileApi.preview(agentId, activePath),
                fileApi.revisions(agentId, activePath).catch(() => []),
            ]);
            setPreview(data);
    ...
    const loadChildDir = async (dirPath: string): Promise<void> => {
        const items = await fileApi.list(agentId, dirPath).catch(() => []);
        const merge = (nodes: WorkspaceFileNode[]): WorkspaceFileNode[] =>
            nodes.map((n) => {
                if (n.path === dirPath && n.is_dir) return { ...n, children: items };
                if (n.children) return { ...n, children: merge(n.children) };
                return n;
            });
        setFileTree((prev) => merge(prev));
    };
    ...
    const isJitDoc = !!activePath && JIT_DOC_EXTS.has(ext);
    ...
    const renderPreview = () => {
        ...
        if (isJitDoc) {
            const downloadUrl = fileApi.downloadUrl(agentId, activePath);
            return (
                            ...
                            onClick={() => {
                                ...
                                setExpandedDirs((prev) => {
                                    ...
                                    if (next.has(node.path)) {
                                        ...
                                        next.add(node.path);
                                        if (!node.children) loadChildDir(node.path);
                                    }
                                    return next;
                                });
                            }}
