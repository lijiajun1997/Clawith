"""AES-128-ECB encryption for WeChat iLink CDN media."""
...
def decrypt_aes_ecb(ciphertext: bytes, key: bytes) -> bytes:
    ...
    if len(ciphertext) % 16 != 0:
        raise ValueError(f"Ciphertext length {len(ciphertext)} not multiple of 16")
...
def decode_aes_key(encoded: str) -> bytes:
    ...
    try:
        ...
        raise ValueError(f"Cannot base64 decode aes_key: {e}") from e
    if len(decoded) == 16:
        return decoded
    if len(decoded) == 32:
        try:
            ...
            pass
    raise ValueError(f"Decoded aes_key unexpected length {len(decoded)}")
