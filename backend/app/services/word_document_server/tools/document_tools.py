...
from app.services.word_document_server.utils.file_utils import check_file_writeable, ensure_docx_extension, create_document_copy
from app.services.word_document_server.utils.document_utils import get_document_properties, extract_document_text, get_document_structure, get_document_xml, insert_header_near_text, insert_line_or_paragraph_near_text
from app.services.word_document_server.core.styles import ensure_heading_style, ensure_table_style
...
async def create_document(filename: str, title: Optional[str] = None, author: Optional[str] = None) -> str:
    ...
    try:
        ...
        # 设置 Normal 样式的中英文字体
        from app.services.word_document_server.core.styles import ensure_normal_style
        ensure_normal_style(doc)
...
async def merge_documents(target_filename: str, source_filenames: List[str], add_page_breaks: bool = True) -> str:
    ...
    """
    from app.services.word_document_server.core.tables import copy_table
